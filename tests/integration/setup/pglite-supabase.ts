...
export async function createTestDatabase(): Promise<PGlite> {
  ...
  // Baseline do Supabase Storage. No projeto real, o schema `storage` e a
  // tabela `storage.buckets` são criados/gerenciados pela plataforma; o
  // PGlite é um Postgres "cru" e não os tem. Sem este stub, a migration
  // `20260826090000_card_attachments_storage.sql` (que registra o bucket
  // privado `card-attachments`) falha com `relation "storage.buckets" does
  // not exist` e derruba TODA a suíte de integração.
  //
  // Só as colunas de fato usadas pela migration são declaradas — mesmo
  // critério minimalista já aplicado a `auth.users` acima. `id` é primary
  // key porque a migration usa `on conflict (id) do update`.
  //
  // Isto stuba apenas o REGISTRO de buckets (metadado SQL). O Storage em si
  // (upload/download de objetos, signed URLs) é um serviço HTTP externo ao
  // Postgres e continua NÃO emulado aqui — ver a seção de limitações no
