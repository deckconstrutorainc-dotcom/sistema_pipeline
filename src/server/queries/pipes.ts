...
export interface CardSummary {
  ...
  commentCount: number;
  /** Itens de checklist concluídos / total. `total = 0` quando não há checklist. */
...
export async function getPipeBoardData(pipeId: string): Promise<PipeBoardData | null> {
  ...
  const [
    ...
    supabase
      ...
      .eq("cards.pipe_id", pipeId),
    // Indicadores do card-tile (comentários e checklist), no mesmo padrão de
    // agregação por contagem já usado para anexos. Só `card_id` e, no
    // checklist, `is_done` — o conteúdo não é necessário para o quadro.
    supabase
      .from("comments")
      .select("card_id, cards!inner(pipe_id)")
      .eq("cards.pipe_id", pipeId),
    supabase
      ...
      .select("card_id, is_done, cards!inner(pipe_id)")
      .eq("cards.pipe_id", pipeId),
  ]);
  ...
  for (const row of (commentsRes.data ?? []) as unknown as { card_id: string }[]) {
    commentCountByCard.set(row.card_id, (commentCountByCard.get(row.card_id) ?? 0) + 1);
  }

  const checklistByCard = new Map<string, { done: number; total: number }>();
  for (const row of (checklistRes.data ?? []) as unknown as {
    card_id: string;
    is_done: boolean;
  }[]) {
    const current = checklistByCard.get(row.card_id) ?? { done: 0, total: 0 };
    current.total += 1;
    if (row.is_done) current.done += 1;
    checklistByCard.set(row.card_id, current);
  }
  ...
  return {
    ...
    cards: ((cardsRes.data ?? []) as unknown as CardRow[]).map((row) => ({
      ...
      checklistDone: checklistByCard.get(row.id)?.done ?? 0,
      checklistTotal: checklistByCard.get(row.id)?.total ?? 0,
