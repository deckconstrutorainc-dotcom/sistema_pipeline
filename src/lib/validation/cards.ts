...
/**
 * Comparador de urgência por prazo, para `Array.prototype.sort`: prazos mais
 * próximos (inclusive já vencidos) primeiro, e itens SEM prazo sempre no fim
 * — nunca antes de algo que tem data. Datas inválidas são tratadas como
 * "sem prazo", igual a `getDueStatus`.
 *
 * Usado pelo dashboard (minhas tarefas / meus cards) e disponível para
 * qualquer listagem que precise da mesma ordenação, para que a noção de
 * "mais urgente" não seja reimplementada em cada tela.
 */
export function compareByDueUrgency(a: string | null | undefined, b: string | null | undefined): number {
  const timeA = toDueTime(a);
  const timeB = toDueTime(b);
...
function toDueTime(dueDate: string | null | undefined): number | null {
