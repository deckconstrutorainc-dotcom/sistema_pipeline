...
describe("compareByDueUrgency", () => {
  it("ordena prazos mais próximos (inclusive vencidos) primeiro", () => {
    const ordered = ["2026-09-01T00:00:00Z", "2026-08-10T00:00:00Z", "2026-08-20T00:00:00Z"].sort(
      compareByDueUrgency,
    );
  ...
  it("mantém itens sem prazo (ou com data inválida) sempre no fim", () => {
    const ordered = [null, "2026-09-01T00:00:00Z", "not-a-date", "2026-08-10T00:00:00Z"].sort(
      compareByDueUrgency,
    );
    expect(ordered.slice(0, 2)).toEqual(["2026-08-10T00:00:00Z", "2026-09-01T00:00:00Z"]);
    expect(ordered.slice(2)).toHaveLength(2);
  });

  it("é estável para prazos iguais e para dois itens sem prazo", () => {
    expect(compareByDueUrgency("2026-08-20T00:00:00Z", "2026-08-20T00:00:00Z")).toBe(0);
    expect(compareByDueUrgency(null, undefined)).toBe(0);
  });
});
